		...
		return {
			CallExpression(node: estree.CallExpression) {
				...
				if (utils.isObjectExpression(firstArgValue)) {
					...
					if (!hasBooleanProp) return;
				} else if (typeof firstArgValue === 'string') {
					if (!utils.booleanAttributes.includes(firstArgValue.toLowerCase())) return;
				} else if (typeof secondArgValue === 'string') {
					if (!utils.booleanAttributes.includes(secondArgValue.toLowerCase())) return;
					...
					if (!nameLooksLikeBooleanAttribute) return;
				}
