...
exports.booleanAttributes = ['allowfullscreen', 'async', 'autofocus', 'autoplay', 'checked', 'compact', 'controls',
...
function getAssignedNode(node, context) {
    ...
    function getAssignedNodeByScope(id, scope) {
        while (scope) {
...
function getVariableAssignments(id, node) {
    ...
    if (isBlockStatement(node) || isProgram(node)) {
        ...
        if (isIdentifier(id)) {
            ...
            assignments = node.body
                ...
                .filter((ae) => isIdentifier(ae.left, id.name))
                .map((ae) => ae.right);
        }
            ...
            if (isIdentifier(obj) && isIdentifier(prop)) {
                varDeclarations = node.body
                    ...
                    .reduce((arr, vd) => arr.concat(vd.declarations), [])
                    .filter(vd => isVariableDeclarator(vd))
                    .filter((vd) => isIdentifier(vd.id, obj.name))
                    .map((vd) => {
            ...
            assignments = node.body
                ...
                .map((es) => es.expression)
                .filter(e => isAssignmentExpression(e))
                .filter((ae) => isMemberExpression(ae.left) && memberExpressionsAreEqual(ae.left, id))
                .map((ae) => ae.right);
        }
...
function memberExpressionsAreEqual(memberExpr1, memberExpr2) {
    if (isIdentifier(memberExpr1.property) && !isIdentifier(memberExpr2.property, memberExpr1.property.name))
        return false;
    if (isIdentifier(memberExpr1.object)) {
        if (!isIdentifier(memberExpr2.object, memberExpr1.object.name))
            return false;
    }
    else if (isMemberExpression(memberExpr1.object)) {
        if (!isMemberExpression(memberExpr2.object))
            return false;
...
function shouldExcludeValue(value, excludedExpressions) {
    if (!Array.isArray(excludedExpressions))
        return false;
    return excludedExpressions
        .some(exclude => {
...
exports.shouldExcludeValue = shouldExcludeValue;
